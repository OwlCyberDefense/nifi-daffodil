...
public abstract class AbstractDaffodilProcessor extends AbstractProcessor {
    ...
    /**
     * These lists collect errors and warnings generated by Daffodil when
     * the processWithDaffodil() function fails as a part of the logDiagnostics()
     * function it calls. These are later written to the failed FlowFile's attributes
     * before it's transferred to REL_FAILURE.
     */
    private static List<String> daffodilErrors = new ArrayList<String>();
    private static List<String> daffodilWarnings = new ArrayList<String>();
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) {
        ...
        try {
            ...
            final FlowFile originalWithDiags = diagnosticsInAttrs(session, original);
            logger.error("Failed to process {} due to {}", new Object[]{original, e});
            session.transfer(originalWithDiags, REL_FAILURE);
        }
    }
    ...
    public static void logDiagnostics(ComponentLog logger, WithDiagnostics withDiags) {
        ...
        for (Diagnostic diag : diags) {
            ...
            if (diag.isError()) {
                ...
                daffodilErrors.add(message);
            } else {
                logger.warn(message);
                daffodilWarnings.add(message);
            }
        }
    }

    private static FlowFile diagnosticsInAttrs(final ProcessSession session, final FlowFile original) {
        FlowFile ff = original;
        for (int i = 0; i < daffodilErrors.size(); i++) {
            ff = session.putAttribute(ff, "daffodil.error.message."+i, daffodilErrors.get(i));
        }
        for (int i = 0; i < daffodilWarnings.size(); i++) {
            ff = session.putAttribute(ff, "daffodil.warn.message."+i, daffodilWarnings.get(i));
        }
        return ff;
    }
}
